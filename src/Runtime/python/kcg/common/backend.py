...
from kcg.driver import DriverBase

KCG_PATH = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
...
def compute_core_version_key():
    ...
    # compiler
    compiler_path = os.path.join(KCG_PATH, 'compiler')
    for lib in pkgutil.iter_modules([compiler_path]):
        with open(lib.module_finder.find_spec(lib.name).origin, "rb") as f:
            contents += [hashlib.sha1(f.read()).hexdigest()]
    # backend
    libtriton_hash = hashlib.sha1()
    with open(os.path.join(KCG_PATH, "_C/libtriton.so"), "rb") as f:
        while True:
            chunk = f.read(1024**2)
            if not chunk:
                break
    ...
    # language
    language_path = os.path.join(KCG_PATH, 'language')
